...
def model_detect(model_name,model_dir):
    ...
    if model_name in models_names:
        if model_name == "xception_model" or model_name == "mobile_net_model":
            ...
            return U2NET(model_name,model_dir)
...
class U2NET:
    ...
    def __init__(self, name="u2net",model_dir="models"):
        import torch
        ...
        try:
            if self.torch.cuda.is_available():
                net.load_state_dict(self.torch.load(os.path.join(self.MODEL_DIR, name, name + '.pth')))
                net.cuda()
            else:
                net.load_state_dict(self.torch.load(os.path.join(self.MODEL_DIR, name, name + '.pth'), map_location="cpu"))
